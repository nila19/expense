...
const addservice = require('../../../api/services/AddService.js');

const fmt = require('../../../api/config/formats');
...
const reInsertTrans = (db, data, done) => {
  const form = {
    ...
    transDt: moment(data.trans.transDt, fmt.YYYYMMDD).format(fmt.DDMMMYYYY),
    accounts: {
  ...
  addservice.addExpensePromise({db: db, log: {error: () => {}}}, form).then((tr) => {
    const mod = {
      id: data.trans.id,
      seq: data.trans.seq,
...
// ====================== Test Cases ======================//
describe('models.deleteservice', function () {
  let db = null;
  ...
  describe('deleteExpense', function () {
    ...
    describe('delete expense for an inactive city', function () {
      ...
      it('should throw an error for delete transaction with inactive city', function (done) {
        deleteservice.deleteExpense({db: db, transId: transId, log: {error: () => {}}}, (err) => {
          if (err) {
