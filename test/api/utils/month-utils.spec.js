...
const mu = require('../../../api/utils/month-utils.js');
...
describe('utils.month-utils', function () {
  describe('getMonth', function () {
    it('should be equal to month obj', function () {
      ...
      expect(mu.getMonth('2017-02-01', false)).to.deep.equal(result);
    });
    it('should be equal to year obj', function () {
      ...
      expect(mu.getMonth('2016-12-31', true)).to.deep.equal(result);
    });
  });
  describe('buildMonthsList', function () {
    it('should add current month with multiple years and sort them..', function (done) {
      ...
      const output = [{id: '2017-12-31', bills: null, aggregate: true, name: '2017', seq: 201713, year: 2017},
      ...
      mu.buildMonthsList(input, console).then((result) => {
